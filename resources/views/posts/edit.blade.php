<x-layout>
<div>        
    <form method="post" action="/posts/{{ $post->id }}/update" enctype="multipart/form-data" class="w-full mt-10">
        @csrf @method('put')
        <div class="w-full md:w-4/5 mx-auto border rounded shadow-md">
        {{-- bg image --}}
        <div style="background-image: url('{{$post->bg_img ? asset('storage/' . $post->bg_img) : asset('/images/bg_img.png') }}')" id="cover" class="bg-center bg-cover bg-no-repeat grid place-items-center w-full h-[420px] mb-8">
            <input
            type="file"
            accept="image/*"
            name="bg_img"
        ...
        <div class="w-full flex flex-col items-center">
            ...
            <div class="flex flex-col w-4/5 mx-auto my-8">
                ...
                @enderror
                <input value="{{ old('title') ?? $post->title }}" type="text" name="title" id="title" class="px-2 py-1.5 rounded border outline-0 focus:border-gray-700 w-full font-bold">
            </div>
        ...
        <div class="mt-8 ml-auto mr-4 mb-4 w-fit">
            <button type="submit" name="submit" class="px-4 py-1.5 font-bold rounded bg-gray-800 hover:bg-gray-900 text-white">Save</button>
        </div>
    </div>
    </form>
