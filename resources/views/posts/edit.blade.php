...
<div>        
    <form method="post" action="/posts/{{ $post->id }}/update" enctype="multipart/form-data" class="w-full mt-10">
        ...
        {{-- bg image --}}
        <div style="background-image: url('{{asset('storage/' . $post->bg_img) ?? asset('/images/bg_img.png') }}')" id="cover" class="bg-contain md:bg-cover bg-no-repeat grid place-items-center w-full h-[420px] mb-8">
            <input
            type="file"
            accept="image/*"
            name="bg_img"
